...
import { Button, Select, Drawer, Radio, Alert } from "antd";

let provider = null;
try {
  provider = new ethers.providers.Web3Provider(window.ethereum, "any");
} catch {
  console.log("no metamask provider, please download metamask extension ");
}
...
function App() {
  ...
  return (
    <>
      ...
      >
        <button
          className="create explore"
          style={{ marginTop: "10px" }}
          value={currentPage == "console" ? "create" : "console"}
          onClick={(e) => setCurrentPage(e.target.value)}
        >
          {currentPage == "console" ? "Create" : "Console"}
        </button>
        <MetaMask
          provider={provider}
          setSigner={setSigner}
      ...
      {currentPage === "create" && (
        <section id="create">
          ...
          <div
            className="parent"
            style={{ padding: "25px", margin: "1% 20%", borderRadius: "20px" }}
          >
            {showAlert && (
              <Alert
            ...
            <div className="container" style={{ margin: "5% auto" }}>
              <div
                style={{ display: "flex", flexFlow: "row wrap", gap: "30px" }}
              >
                <input
                  className="input"
                  disabled={isCreating == 0 ? true : false}
                  autoComplete="lodahasan"
                  value={tokenName}
                  onChange={(e) => {
          ...
          >
            <div style={{ display: "flex", flexFlow: "column", gap: "10px" }}>
              ...
              >
                ...
                ></input>
                <button className="create explore" onClick={() => getImages()}>
                  Search
                </button>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                <input
                  ...
                  className="radio"
                  type="radio"
                  checked={imageEndPoint === "unsplash"}
                  onClick={(e) => {
                    setImageEndPoint("unsplash");
                  }}
                />
                ...
                <input
                  ...
                  className="radio"
                  type="radio"
                  checked={imageEndPoint === "dalle"}
                  onClick={() => {
                    setImageEndPoint("dalle");
                  }}
                />
              ...
              >
                {imageData?.length > 0 ? (
                  ...
                  <p style={{ color: "red", fontSize: "10pxs" }}>
                    No Result Found
                  </p>
                ) : finding == 10 ? (
                  <>
      ...
      {currentPage === "console" && (
        <>
          <section id="console">
            {tokenData?.length > 0 ? (
              <Card tokens={tokenData} signer={signer} />
            ) : (
              <div style={{ position: "absolute", top: "30%", left: "30%" }}>
                <p style={{ color: "red", fontSize: "40px" }}>
                  {" "}
                  {!isActive ? "Connect MetaMask First !" : "No Token Found"}
                </p>
              </div>
            )}
          </section>
      ...
      {currentPage == "test" && <section id="test"></section>}
    </>
  );
}
